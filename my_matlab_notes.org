...
1. Open OnDemand (interactive GUI) – Recommended for beginners.
2. Slurm Batch Script – Recommended for production HPC jobs.
3. Local MATLAB Cluster Profile (batch) – Advanced workflow.
...
2. *Requested compute size* (MATLAB workers)
   ...
   - Some partitions enforce policies on job size. For example, on =orfoz= you may
     be required to request 56×n (where n is integer) cores.
...
2. Select *Interactive Apps → Desktop*.
