...
commonSubmitArgs = '-D /arf/scratch/$USER';
...
%MW: Do not make memory required for job submission.  Only
% communicating jobs are supported; therefore, in totality, we might
% have enough memory for each worker.
% Memory required per CPU
commonSubmitArgs = iAppendArgument(commonSubmitArgs, ap, ...
    'MemPerCPU', 'char', '--mem-per-cpu=%s');

% Number of nodes (required for GPUs)
nnodes = validatedPropValue(ap, 'NumNodes', 'double', 0);
if nnodes>0
    commonSubmitArgs = sprintf('%s -N %d', commonSubmitArgs, nnodes);
end
