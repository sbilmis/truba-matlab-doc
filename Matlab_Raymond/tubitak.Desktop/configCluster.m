...
% Check to make sure that Parallel Computing Toolbox is installed before proceeding.
installed_toolboxes = ver;
isPCTInstalled = any(strcmp({installed_toolboxes.Name}, 'Parallel Computing Toolbox'));

if ~isPCTInstalled
    error('Parallel Computing Toolbox is not installed. Please install and try again.')
end
...
selected = false;
%MW User might not have access to all clusters
disp("Ensure you pick a cluster you have access to.")
...
% Pull the cluster operating system from the .conf file if set
if isfield(def,'OperatingSystem') && ~isempty(def.OperatingSystem)
    c.OperatingSystem = def.OperatingSystem;
end
...
% Set ClusterMatlabRoot if it is present.
...
% Save Profile
c.saveAsProfile(profile,'Description', profile)
